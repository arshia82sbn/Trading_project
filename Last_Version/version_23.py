...
def generate_signals(df_org, year, month, indicator_counting, balance):
...
# MA Cross
    df['MA_Signal'] = 0
    periods = inputs['moving_average']
    
    # Fast from down cross slow
    MA_buy_signal = (df[f'MA {periods[0]}'] > df[f'MA {periods[1]}']) & (df[f'MA {periods[0]}'].shift(1) <= df[f'MA {periods[1]}'].shift(1))
    
    # Fast from up cross slow
    MA_sell_signal = (df[f'MA {periods[0]}'] < df[f'MA {periods[1]}']) & (df[f'MA {periods[0]}'].shift(1) >= df[f'MA {periods[1]}'].shift(1))
...
#MACD or stoch signals
    trade_lock = False  # Trade lock to prevent multiple trades within one MA_Signal period
    macd_state = 1  # State variable for MACD (0 = idle, 1 = waiting for condition)
    stoch_state = 1
    df['MACD_Signal'] = 0
    df['Stoch_Signal'] = 0
    for i in range(1, len(df)):
        if df['MA_Signal'].iloc[i] == 1 and not trade_lock:  # MA Buy signal and no trade lock
            # macd
            if macd_state == 1 and df['MACD_Histogram'].iloc[i] > 0:  # when macd above zero
                macd_state = 2  # Waiting for MACD to cross back below zero
            elif macd_state == 2 and df['MACD_Histogram'].iloc[i] < 0:  # MACD crossed back below zero
                df.loc[i, 'MACD_Signal'] = 1  # Final Buy signal
                trade_lock = True  # Lock trade
                macd_state = 1  # Reset MACD state
            # stoch
            if stoch_state == 1 and df['Stochastic %K'].iloc[i] > 50:
                stoch_state= 2
            elif stoch_state == 2 and df['Stochastic %K'].iloc[i] < 50:
                df.loc[i, 'Stoch_Signal'] = 1  # Final Buy signal
                trade_lock = True
                stoch_state = 1

        elif df['MA_Signal'].iloc[i] == -1 and not trade_lock:  # MA Sell signal and no trade lock

            if macd_state == 1 and df['MACD_Histogram'].iloc[i] < 0:  # MACD crossed below zero
                macd_state = 2  # Waiting for MACD to cross back above zero
            elif macd_state == 2 and df['MACD_Histogram'].iloc[i] > 0:  # MACD crossed back above zero
                df.loc[i, 'MACD_Signal'] = -1  # Final Sell signal
                trade_lock = True  # Lock trade
                macd_state = 1  # Reset MACD state
            # stoch
            if stoch_state == 1 and df['Stochastic %K'].iloc[i] > 50:
                ...
                df.loc[i, 'Stoch_Signal'] = -1  # Final Sell signal
                trade_lock = True
                stoch_state = 1
        # Reset trade lock when MA_Signal changes
